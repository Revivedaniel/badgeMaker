...
namespace CatWorx.BadgeMaker
{
    class Util
    {
        ...
        public static void MakeBadges(List<Employee> employees)
        {
            ...
            // instance of WebClient is disposed after the code in the block is executed
            using (WebClient client = new WebClient())
            {
                for (int i = 0; i < employees.Count; i++)
                {
                    // This opens a stream to the image on the web
                    // && creates a new image from the stream
